...
export const Register = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    email: "",
    password: "",
    confirmPassword: "", // Added to match likely backend requirement or we handle validation
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register({
        ...formData,
        confirmPassword: formData.password // Ensure we send this if backend checks it strictly equal
      });
      // On success, redirect to login
      navigate("/login");
    } catch (err: any) {
        // Backend returns standard error message
  ...
  return (
    <AuthLayout title="Capturing Moments,<br/>Creating Memories">
      <div className="max-w-md w-full mx-auto space-y-8 animate-slide-up">
        <div className="space-y-2">
            <h2 className="text-3xl font-bold tracking-tight">Create an account</h2>
            <p className="text-gray-400">
                Already have an account?{" "}
                <Link to="/login" className="text-[#a78bfa] hover:text-[#c4b5fd] transition-colors underline-offset-4 hover:underline">
                    Log in
                </Link>
            </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center gap-2">
                <input 
                    type="checkbox" 
                    id="terms" 
                    className="w-4 h-4 rounded border-gray-600 bg-[#2d2d3f] text-[#6b46c1] focus:ring-[#6b46c1] accent-[#6b46c1]"
                    required
                />
                <label htmlFor="terms" className="text-sm text-gray-400 select-none">
                    I agree to the <a href="#" className="text-gray-300 underline">Terms & Conditions</a>
                </label>
            </div>
        ...
        <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-[#1c1c2e] px-2 text-gray-500">Or register with</span>
            </div>
        </div>
